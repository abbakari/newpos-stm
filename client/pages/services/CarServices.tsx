...
interface ServiceOrder {
  ...
  status: 'Draft' | 'Scheduled' | 'In Progress' | 'Quality Check' | 'Completed' | 'Cancelled' | 'On Hold';
...
const mockServicePackages: ServicePackage[] = [
  ...
  },
  {
    ...
    services: ['Oil Change', 'Filter Replacement', 'Brake Inspection', 'Tire Check'],
    estimatedDuration: 120,
    price: 200000,
    category: 'Maintenance',
  },
  {
    ...
    services: ['Oil Change', 'Filter Replacement', 'Brake Inspection', 'Tire Check', 'AC Service', 'Engine Diagnostic'],
...
export default function CarServices() {
  ...
  const filteredOrders = serviceOrders.filter(order => {
    ...
    const matchesTechnician = selectedTechnician === 'all' || order.assignedTechnician === selectedTechnician;
  ...
  const scheduledOrders = serviceOrders.filter(order => order.status === 'Scheduled').length;
  const completedToday = serviceOrders.filter(order => 
    order.status === 'Completed' && order.completedDate === new Date().toISOString().split('T')[0]
  ).length;
  const availableTechnicians = technicians.filter(tech => tech.status === 'Available').length;
  const totalRevenue = serviceOrders.filter(order => order.status === 'Completed').reduce((sum, order) => sum + order.totalCost, 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Car Service Management</h1>
          <p className="text-muted-foreground">
            Manage vehicle services, track progress, and coordinate technician assignments
          </p>
        </div>
        ...
        <TabsContent value="orders">
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-lg">Active Service Orders</CardTitle>
                  <CardDescription>
                        ...
                        <TableRow key={order.id} className="hover:bg-accent/50">
                          <TableCell>
                            <div>
                              ...
                              <p className="text-sm text-muted-foreground">Job: {order.jobCardId}</p>
                          ...
                          <TableCell>
                            <div>
                              ...
                              <p className="text-sm text-muted-foreground">{order.vehicleInfo.plateNumber}</p>
                              <div className="flex items-center gap-1 mt-1">
                          ...
                          <TableCell>
                            <div>
                              ...
                              <p className="text-sm text-muted-foreground">{order.services.length} services</p>
                              <p className="text-sm font-medium text-success">{formatCurrency(order.totalCost)}</p>
                            </div>
                          </TableCell>
                          <TableCell>
                            <div className="space-y-2">
                              ...
                              <div className="flex items-center gap-1 text-xs text-muted-foreground">
                                ...
                                <span>
                                  {order.actualDuration ? formatDuration(order.actualDuration) : formatDuration(order.estimatedDuration)} est.
                                </span>
                              </div>
                            </div>
                          </TableCell>
                              ...
                              <Dialog>
                                ...
                                <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
                                  <DialogHeader>
                                    ...
                                    <DialogDescription>
                                      Complete service order information and progress tracking
                                    </DialogDescription>
                                  </DialogHeader>
                                  {selectedOrder && (
                                    <div className="space-y-6">
                                      ...
                                      <div className="grid gap-4 md:grid-cols-3">
                                        ...
                                        <Card>
                                          ...
                                          <CardContent className="space-y-2">
                                            ...
                                            <p className="text-sm">Year: {selectedOrder.vehicleInfo.year}</p>
                                            <p className="text-sm">Plate: {selectedOrder.vehicleInfo.plateNumber}</p>
                                            <p className="text-sm">Color: {selectedOrder.vehicleInfo.color}</p>
                                            <p className="text-sm">Mileage: {selectedOrder.vehicleInfo.mileage.toLocaleString()} km</p>
                                          </CardContent>
                                        </Card>
                                        ...
                                        <Card>
                                          ...
                                          <CardContent className="space-y-2">
                                            ...
                                            <p className="text-sm">{selectedOrder.servicePackage}</p>
                                      ...
                                      <Card>
                                        <CardHeader>
                                          <div className="flex items-center justify-between">
                                            ...
                                            <div className="flex items-center gap-2">
                                              <Badge variant="outline">{calculateProgress(selectedOrder.services)}% Complete</Badge>
                                        ...
                                        <CardContent>
                                          <div className="space-y-4">
                                            <Progress value={calculateProgress(selectedOrder.services)} className="h-3" />
                                                  ...
                                                  <div className="flex-1">
                                                    <div className="flex items-center gap-3">
                                                      <div className={cn(
                                                        ...
                                                        service.status === 'Completed' ? 'bg-success text-success-foreground' :
                                                        service.status === 'In Progress' ? 'bg-info text-info-foreground' :
                                                      ...
                                                      <div>
                                                        ...
                                                        <div className="flex items-center gap-3 mt-1">
                                                          ...
                                                          <span className="text-xs text-muted-foreground">
                                                            Est. {formatDuration(service.estimatedTime)}
                                                          </span>
                                      ...
                                      <div className="grid gap-4 md:grid-cols-2">
                                          ...
                                          <CardContent className="space-y-3">
                                            <div className="flex items-center gap-3">
                                              ...
                                              <div>
                                                ...
                                                <p className="text-xs text-muted-foreground">{selectedOrder.createdDate}</p>
                                              </div>
                                            </div>
                                            <div className="flex items-center gap-3">
                                              ...
                                              <div>
                                                ...
                                                <p className="text-xs text-muted-foreground">{selectedOrder.scheduledDate}</p>
                                              </div>
                                            </div>
                                            {selectedOrder.startTime && (
                                              <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                  ...
                                                  <p className="text-xs text-muted-foreground">{selectedOrder.startTime}</p>
                                                </div>
                                              </div>
                                            )}
                                            {selectedOrder.completedDate && (
                                              <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                  ...
                                                  <p className="text-xs text-muted-foreground">{selectedOrder.completedDate}</p>
                                                </div>
                                              </div>
                                            )}
                                          </CardContent>
                                        ...
                                        <Card>
                                          <CardHeader>
                                            <CardTitle className="text-base">Notes & Requests</CardTitle>
                                          </CardHeader>
                                          <CardContent className="space-y-3">
                                            {selectedOrder.customerRequests && (
                                              <div>
                                                <p className="text-sm font-medium">Customer Requests:</p>
                                                <p className="text-sm text-muted-foreground">{selectedOrder.customerRequests}</p>
                                              </div>
                                            )}
                                            {selectedOrder.notes && (
                                              <div>
                                            ...
                                            <div>
                                              ...
                                              <p className="text-sm text-muted-foreground">{selectedOrder.assignedTechnician}</p>
                                            </div>
                                          </CardContent>
                                        </Card>
                                      </div>
                    ...
                    <CardContent className="space-y-4">
                      <div className="space-y-2">
                        <div className="flex justify-between text-sm">
                          <span>Workload:</span>
                          <span>{technician.currentJobs}/{technician.maxJobs}</span>
                        </div>
                        <Progress value={(technician.currentJobs / technician.maxJobs) * 100} className="h-2" />
                      </div>
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          ...
                          <p className="font-medium">{formatDuration(pkg.estimatedDuration)}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">Price:</p>
                          <p className="font-medium text-success">{formatCurrency(pkg.price)}</p>
                        </div>
                      </div>
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                  ...
                  <Card>
                    <CardHeader className="pb-3">
                      <CardTitle className="text-base">Morning Schedule (8:00 - 12:00)</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-3">
                      {serviceOrders.filter(order => order.status === 'Scheduled' || order.status === 'In Progress').slice(0, 3).map((order) => (
                        <div key={order.id} className="flex items-center justify-between p-3 border rounded-lg">
                          <div>
                            <p className="font-medium">{order.customerName}</p>
                            <p className="text-sm text-muted-foreground">
                              {order.vehicleInfo.make} {order.vehicleInfo.model} - {order.serviceType}
                            </p>
                            <div className="flex items-center gap-2 mt-1">
                              <Badge variant="outline" className="text-xs">{order.assignedTechnician}</Badge>
                              <span className="text-xs text-muted-foreground">{formatDuration(order.estimatedDuration)}</span>
                            </div>
                          </div>
                  ...
                  <Card>
                    <CardHeader className="pb-3">
                      <CardTitle className="text-base">Afternoon Schedule (13:00 - 17:00)</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-3">
                      {serviceOrders.filter(order => order.status === 'Scheduled').slice(0, 2).map((order) => (
                        <div key={order.id} className="flex items-center justify-between p-3 border rounded-lg">
                          <div>
                            <p className="font-medium">{order.customerName}</p>
                            <p className="text-sm text-muted-foreground">
                          ...
                          <div className="text-right">
                            ...
                            <p className="text-xs text-muted-foreground mt-1">14:00</p>
                          </div>
                        </div>
                      ))}
                    </CardContent>
