...
const mockCustomers: Customer[] = [
  { id: 'CUST-001', name: 'John Doe', phone: '+256 700 123 456', type: 'Personal' },
  { id: 'CUST-002', name: 'Jane Smith', phone: '+256 702 456 789', type: 'Personal' },
  { id: 'CUST-003', name: 'Express Taxi Services', phone: '+256 704 789 012', type: 'Private' },
];
...
export default function TireServices() {
  ...
  const calculateTotals = (items: SaleItem[]) => {
    const subtotal = items.reduce((sum, item) => sum + item.total + (item.installationFee || 0), 0);
    const tax = subtotal * 0.18; // 18% VAT
    const total = subtotal + tax;
    return { subtotal, tax, total };
  };

  const addItemToSale = (product: TireProduct, quantity: number, includeInstallation: boolean) => {
  ...
  const todaysSales = sales.filter(sale => sale.date === new Date().toISOString().split('T')[0]).length;
  const totalRevenue = sales.filter(sale => sale.status === 'Completed').reduce((sum, sale) => sum + sale.total, 0);
  const pendingSales = sales.filter(sale => ['Draft', 'Confirmed', 'In Progress'].includes(sale.status)).length;
                                  ...
                                  <div className="space-y-6">
                                    ...
                                    <div className="grid gap-4 md:grid-cols-3">
                                      <div>
                                        ...
                                        <p className="text-sm text-muted-foreground">{selectedSale.vehicleInfo}</p>
                                      </div>
                                      <div>
                                        <h4 className="font-medium mb-2">Service</h4>
                                        <Badge variant="outline">{selectedSale.serviceType}</Badge>
                                        ...
                                        <TableBody>
                                          {selectedSale.items.map((item) => (
                                            <TableRow key={item.id}>
                                              ...
                                              <TableCell>
                                                {item.installationFee ? formatCurrency(item.installationFee) : '-'}
                                              </TableCell>
                                              <TableCell>
                                                {formatCurrency(item.total + (item.installationFee || 0))}
                                              </TableCell>
                                            </TableRow>
                                          ))}
                                        </TableBody>
        ...
        <TabsContent value="products">
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-lg">Available Tire Products</CardTitle>
                  <CardDescription>
                  ...
                  <Card key={product.id} className="hover:shadow-lg transition-shadow">
                    ...
                    <CardContent className="space-y-3">
                      ...
                      <div className="flex gap-2 pt-2">
                        <Button size="sm" className="flex-1" disabled={product.currentStock === 0}>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Available service options for tire sales</CardDescription>
              </CardHeader>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between p-3 border rounded-lg">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Customer purchases tires without installation</p>
                    </div>
                  ...
                  <div className="flex items-center justify-between p-3 border rounded-lg">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Tire purchase with professional installation</p>
                      <p className="text-xs text-muted-foreground">Installation fee: UGX 25,000 per tire</p>
                    </div>
                    <Badge>Popular</Badge>
                  </div>
                  <div className="flex items-center justify-between p-3 border rounded-lg">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Installation of customer-provided tires</p>
                      <p className="text-xs text-muted-foreground">Service fee: UGX 25,000 per tire</p>
                    </div>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Extra services available with tire installation</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-3">
                  <div className="flex items-center justify-between p-3 border rounded-lg">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">UGX 15,000 per wheel</p>
                    </div>
                    <Button variant="outline" size="sm">
                  ...
                  <div className="flex items-center justify-between p-3 border rounded-lg">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">UGX 50,000 per vehicle</p>
                    </div>
                    <Button variant="outline" size="sm">
                      <Settings className="h-4 w-4" />
                    </Button>
                  ...
                  <div className="flex items-center justify-between p-3 border rounded-lg">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Complimentary service</p>
                    </div>
                    <Button variant="outline" size="sm">
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setNewSale(prev => ({ ...prev, vehicleInfo: e.target.value }))}
                />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select value={newSale.serviceType} onValueChange={(value) => 
                ...
                <SelectContent>
                  ...
                  <SelectItem value="Sale + Installation">Sale + Installation</SelectItem>
                  <SelectItem value="Installation Only">Installation Only</SelectItem>
                </SelectContent>
              </Select>
            </div>
                    ...
                    <div key={item.id} className="flex items-center justify-between p-3 border rounded-lg">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          ...
                          {item.installationFee && ` + Installation: ${formatCurrency(item.installationFee)}`}
                        </p>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="font-medium">{formatCurrency(item.total + (item.installationFee || 0))}</span>
                        <Button variant="ghost" size="sm">
